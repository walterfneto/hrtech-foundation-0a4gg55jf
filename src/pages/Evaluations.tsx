...
export default function Evaluations() {
  ...
  const loadData = async () => {
    setLoading(true)
    try {
      const [tpls, cycs, emps] = await Promise.all([fetchTemplates(), fetchCycles(), fetchUsers()])
      setTemplates(tpls)
      setCycles(cycs)
      setEmployees(emps)
      if (cycs.length > 0) {
  ...
  const openForm = (tpl: EvaluationTemplate, cyc: EvaluationCycle, emp: Usuario) => {
    setSelectedTpl(tpl)
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
                  ...
                  {employees
                    ...
                    .map((u) => (
                      ...
                      >
                        ...
                        <Button size="sm" onClick={() => openForm(activeTpl, activeCycle, u)}>
                          Avaliar
                        </Button>
                      </div>
                    ))}
      ...
      {selectedTpl && selectedCycle && selectedEmployee && (
        <EvaluationFormDialog
          ...
          targetUserName={selectedEmployee.nome}
